...
do
	...
	if [[ "$sample" = "${samples[0]}" ]]; then #process for ctrl
		annotatePeaks.pl ${workdir}/${sample}_GG_ctrl_stopsite.bed hg38 -annStats ${workdir}/distribution_onlyGG/ctrl_elementStat.txt > ${workdir}/distribution_onlyGG/ctrl_annotatePeaks_Stats.txt
	...
	annotatePeaks.pl ${workdir}/${sample}_GG_treat.final_stopsite.bed hg38 -annStats ${workdir}/distribution_onlyGG/${sample}_elementStat.txt > ${workdir}/distribution_onlyGG/${sample}_annotatePeaks_Stats.txt
...
Rscript snk_script/7.1.plot_subtype_persample_onlyGG.r -i ${workdir}/distribution_onlyGG -o ${workdir}/distribution_onlyGG/annotation.pdf
