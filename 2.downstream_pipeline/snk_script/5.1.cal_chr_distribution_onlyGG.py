...
def main(workdir):
   ...
   with open(f'{workdir}/stopsite_bedlist_onlyGG.txt','r') as f:
      for line in f:
         try:
            aline = line.strip()
            attri = aline[15:].split('_')
            trt, con = attri[0], attri[1]
            dff = pl.read_csv(aline, has_header=True, separator='\t', new_columns=['chr','start','end','trt','GG','strand']).drop(['trt','GG','strand'])
   ...
   res.with_columns(
      ...
      pl.col('condition'),
      pl.col('enrichment')
   ]).write_csv(f'{workdir}/stopsite_enrichment_onlyGG.txt',include_header=True, separator='\t')
      #pl.col('antibody'),
