...
do
	...
	if [[ "$sample" = "${samples[0]}" ]]; then #process for ctrl
		annotatePeaks.pl ${workdir}/${sample}_ctrl_stopsite.bed hg38 -annStats ${workdir}/distribution/ctrl_elementStat.txt > ${workdir}/distribution/ctrl_annotatePeaks_Stats.txt
	...
	annotatePeaks.pl ${workdir}/${sample}_treat.final_stopsite.bed hg38 -annStats ${workdir}/distribution/${sample}_elementStat.txt > ${workdir}/distribution/${sample}_annotatePeaks_Stats.txt
...
Rscript snk_script/7plot_subtype_persample.r -i ${workdir}/distribution -o ${workdir}/distribution/annotation.pdf
